...
9. SELECT b.fname AS funcionario ,e.fname AS supervisor FROM EMPLOYEE AS e INNER JOIN EMPLOYEE AS b ON e.ssn=b.superssn; -- ERRADO
10. SELECT funcionario FROM (SELECT b.fname AS funcionario ,e.fname AS supervisor FROM EMPLOYEE AS e INNER JOIN EMPLOYEE A
S b ON e.ssn=b.superssn) AS func_sup WHERE supervisor = 'Franklin'; -- ERRADO

11. SELECT d.dname, l.dlocation FROM department AS d INNER JOIN dept_locations AS l ON d.dnumber=l.dnumber; -- ERRADO
12. SELECT dname FROM (SELECT d.dname, l.dlocation FROM department AS d INNER JOIN dept_locations AS l ON d.dnumber=l.dnumber) AS depar_locat WHERE dlocation LIKE 'S%'; --ERRADO